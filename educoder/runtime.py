...
class EduCoder:
    ...
    def secret_env_items(self):
        return self.detected_secret_env_items()
    ...
    def parse(raw):
        ...
        if "<tool" in raw and ("<final>" not in raw or raw.find("<tool") < raw.find("<final>")):
            ...
            if payload is not None:
                return "tool", payload
            return "retry", EduCoder.retry_notice()
        # Fallback: some models output <tool_name>{json}</tool_name> instead of
        # the standard <tool>{"name":"tool_name","args":{...}}</tool> format.
        _BARE_TOOL_RE = re.compile(
            r"<(list_files|read_file|search|run_shell|write_file|patch_file|delegate|run_sandbox_code)"
            r"\s*>(.*?)</\1>",
            re.S,
        )
        bare_match = _BARE_TOOL_RE.search(raw)
        if bare_match:
            tool_name = bare_match.group(1)
            body = bare_match.group(2).strip()
            try:
        ...
        # Fallback: models that use <delegate task="..." max_steps="N" ...> with
        # possibly malformed XML (missing closing > on the opening tag).
        delegate_attr_match = re.search(r"<delegate\s+task\s*=\s*\"([^\"]+)\"(.*?)(?:/>|</delegate>)", raw, re.S)
        if delegate_attr_match:
            task = delegate_attr_match.group(1)
            attrs_text = delegate_attr_match.group(2)
            max_steps_match = re.search(r"max_steps\s*=\s*\"?(\d+)", attrs_text)
            args = {"task": task}
            if max_steps_match:
                args["max_steps"] = int(max_steps_match.group(1))
            return "tool", {"name": "delegate", "args": args}
        if "<final>" in raw:
